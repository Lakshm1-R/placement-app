...
const ManageDrives = () => {
    ...
    try {
      ...
      if (driveEnded && drive.selectionRounds && drive.selectionRounds.length > 0) {
        ...
        const response = await fetch(
          `http://localhost:5000/api/job-drives/${jobDriveId}/last-round-students`,
          {
            method: "GET",
            headers: {
  ...
  const fetchDepartmentJobDrives = async (department) => {
    try {
      ...
      const response = await axios.get(
        `http://localhost:5000/api/job-drives/department/${encodeURIComponent(
          department
        )}`,
        {
          headers: {
  ...
  const fetchJobDriveStudents = async (jobDriveId, companyName) => {
    try {
      const token = localStorage.getItem("token");
      const response = await axios.get(
        `http://localhost:5000/api/job-drives/${jobDriveId}/students`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
            "Content-Type": "application/json",
