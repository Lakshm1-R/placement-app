...
const PlacementAnalytics = () => {
  ...
  const fetchBatches = async () => {
    try {
      ...
      const response = await axios.get(
        "http://localhost:5000/api/placement-analytics/batches",
        {
  ...
  const fetchAnalytics = async (batch) => {
    try {
      ...
      const token = localStorage.getItem("token");
      const response = await axios.get(
        `http://localhost:5000/api/placement-analytics/${batch}`,
        {
          headers: { Authorization: `Bearer ${token}` },
        }
      );
  ...
  const handleFileUpload = async () => {
    ...
    try {
      ...
      const token = localStorage.getItem("token");
      await axios.post(
        "http://localhost:5000/api/placement-analytics/upload",
        formData,
        {
          headers: {
  ...
  const confirmDeleteAnalytics = async () => {
    try {
      const token = localStorage.getItem("token");
      await axios.delete(
        `http://localhost:5000/api/placement-analytics/${selectedBatch}`,
        {
          headers: { Authorization: `Bearer ${token}` },
        }
      );
